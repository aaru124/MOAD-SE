...
class MovieDetailsScreen extends StatefulWidget {
  ...
  MovieDetailsScreen({super.key,required this.user,required this.movie,required this.total});
  @override
...
class _MovieDetailsScreenState extends State<MovieDetailsScreen> {
  ...
  CollectionReference event = FirebaseFirestore.instance.collection('attended-event');
  CollectionReference ecc = FirebaseFirestore.instance.collection('total-ecc');
  CollectionReference event_list = FirebaseFirestore.instance.collection('events');
...
Future<void> updateDisabled() {
      int ind=movie.srno;
      // Calling the collection to add a new user
      return event_list.doc("event-${ind}")
          //adding to firebase collection
          .update({
            //Data added in the form of a dictionary into the document.
            'disabled':true,
        ...
        leading: IconButton(
          icon: Icon(Icons.arrow_back, color: Colors.black),
                 onPressed: () => Navigator.push(
                   context, MaterialPageRoute(builder: (context) => ECCHome(user:user,total:total))),
        ...
        actions: <Widget>[
          Padding(
            ...
            child: ElevatedButton(
              ...
              child: Text("Attend",style: TextStyle(color: b),),
            ),
          ),
        ],
      ),
